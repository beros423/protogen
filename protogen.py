...
def create_ot2_section(protocol, sheet_names, destination_names, key_prefix):
    ...
    with st.expander("OT2 convert:", expanded=False):
        metadata = st.text_area(
            value="""'protocolName': 'Custom Protocol',\n'robotType': 'OT-2'""", 
            label="Metadata", 
            key=f'{key_prefix}_metadata_ot2'
        ).replace("\n", "\n    ")
...
if "design2_list" not in st.session_state:
    st.session_state.design2_list = None
if "commons" not in st.session_state:
    st.session_state.commons = None
if "lv2_commons" not in st.session_state:
    st.session_state.lv2_commons = None
if "vols" not in st.session_state:
    st.session_state.vols = None
if "total_vol" not in st.session_state:
    st.session_state.total_vol = None
...
steps = ["Input Sources", "Design", "Set Commons", "Results"]
for step in steps:
    if st.sidebar.button(step, key=f"nav_{step}", use_container_width=True):
...
# ============================================================================
if st.session_state.current_step == "Input Sources":
    st.header("Step 1: Input Sources")
...
# ============================================================================
    ...
    if st.session_state.sources is None:
        ...
        st.header("Step 2: Volume setting")
        ...
        st.write("")
        st.write("")
        st.header("Step3: Design TU assemblies")
...
# STEP 3: SET COMMONS
# ============================================================================
elif st.session_state.current_step == "Set Commons":
    if st.session_state.sources is None or st.session_state.design_df is None:
        ...
        with col1:
            if st.button("← Go to Input Sources"):
                ...
                st.rerun()
        with col2:
            if st.button("← Go to Design"):
                st.session_state.current_step = "Design"
                st.rerun()
    else:
        sources = st.session_state.sources.copy()
        design_df = st.session_state.design_df
        design2_list = st.session_state.design2_list
        vols = st.session_state.vols
        
        st.header("Step 3: Set Commons")
...
# ============================================================================
    ...
    if (st.session_state.sources is None or st.session_state.design_df is None or 
        ...
        sources = st.session_state.sources.copy()
        design_df = st.session_state.design_df.copy()
        design2_list = st.session_state.design2_list
        vols = st.session_state.vols
        commons = st.session_state.commons
        lv2_commons = st.session_state.lv2_commons
        total_vol = st.session_state.total_vol
        ...
        if lv1_protocol is not None:
            ...
            user_defined_groups = list(set([item[0] for item in design2_list]))
            for comb in design2_list: # group
                group, items = comb
                group_name = f"Group_{group}"
                row_design = [{'name': f"{item['P']}-{item['C']}-{item['T']}-{item['N']}", 'volume': total_vol, 'note': group_name} for item in items]
            ...
            if convert_option == "OT2":
                sheet_names = sources['plate'].unique().tolist()
                
                # OT2 convert for Lv1
                with st.expander("OT2 Lv1 Protocol", expanded=False):
                    st.write("#### Lv1 OT2 Script")
                    metadata_lv1 = st.text_area(
                        value="""'protocolName': 'Lv1 Protocol',\n'robotType': 'OT-2'""", 
                        label="Metadata", 
                        key='lv1_metadata_ot2'
                    ).replace("\n", "\n    ")
                    
                    requirements_lv1 = st.text_area(
                        value='"robotType": "OT-2", "apiLevel": "2.17"', 
                        label="Requirements", 
                        key='lv1_requirements'
                    )
                    
                    plate_posit_lv1, plate_types_lv1 = create_ot2_labware_settings(sheet_names, lv1_destination_names, "lv1")
                    converted_protocol_lv1 = protocol_to_ot2_script(lv1_protocol, metadata_lv1, requirements_lv1, plate_posit_lv1)
                    st.code(converted_protocol_lv1, language='python')
                
                # OT2 convert for Lv2
                with st.expander("OT2 Lv2 Protocol", expanded=False):
                    st.write("#### Lv2 OT2 Script")
                    metadata_lv2 = st.text_area(
                        value="""'protocolName': 'Lv2 Protocol',\n'robotType': 'OT-2'""", 
                        label="Metadata", 
                        key='lv2_metadata_ot2'
                    ).replace("\n", "\n    ")
                    
                    requirements_lv2 = st.text_area(
                        value='"robotType": "OT-2", "apiLevel": "2.17"', 
                        label="Requirements", 
                        key='lv2_requirements'
                    )
                    
                    plate_posit_lv2, plate_types_lv2 = create_ot2_labware_settings(sheet_names, lv2_destination_names, "lv2")
                    converted_protocol_lv2 = protocol_to_ot2_script(lv2_protocol, metadata_lv2, requirements_lv2, plate_posit_lv2)
